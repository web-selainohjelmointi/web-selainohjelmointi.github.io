var pheromones = {
    ...
    timer: null,
    IDLE_MODE: false,
    TICK_LENGTH: 10000, // as milliseconds
    IDLE_TICK_THRESHOLD: 10, // as ticks, i.e. 60 is a couple of minutes 
    ...
    init: function(options) {
        ...
        $("body").find("*").each(function() {
            if (!$(this).is("p") && !$(this).is("pre") && !$(this).is("img")) {
                return;
            }
...
//        console.log("fetch user data");
    ...
    bindWindowActivity: function() { // requires Visiblity.js
    ...
    scroll: {
        ...
        bindScroll: function() {
            $(window).scroll(function() {
                ...
                var firstVisible = pheromones.fn.getTopmostVisibleElement(state.FIRST_VISIBLE, state.PAGETOP_Y > window.pageYOffset);

                if (firstVisible >= 0) {
                    state.FIRST_VISIBLE = firstVisible;
                    state.LAST_VISIBLE = pheromones.fn.getBottommostVisibleElement(state.FIRST_VISIBLE);
                }
    ...
    storage: {
        ...
        init: function() {
            if (!pheromones.storage.hasLocalStorageSupport() || !localStorage['pheromones.data']) {
                pheromones.storage.data = [];
                return;
            }
        ...
        addLocation: function(eventType) {
            ...
            if (v.CURRENT_LAST < v.FIRST_VISIBLE
            ...
            if (v.CURRENT_FIRST === v.FIRST_VISIBLE && v.CURRENT_LAST === v.LAST_VISIBLE) {
                v.IDLE_TICKS++;
            } else {
                v.IDLE_TICKS = 0;
                pheromones.IDLE_MODE = false;
        ...
        getTopmostVisibleElement: function(index, forward) {
            ...
            while (true) {
                ...
                if (pheromones.fn.elementVisible(element[0])) {
                    ...
                    if (lastVisibleElementIndex === -1 || lastVisibleElementIndex > index) {
